...
Person::Person(string p, Date d) : name(p), bday(d){ CalcAge();}
Person::Person(string p, int m, int d, int y) : name(p), bday(Date(m, d, y, "(mdy)")){CalcAge();}
...
string Person::GetNameAge(){
   std::stringstream ss;
   ss << "name: " << name << ", age: " << age << " year" << (age == 1 ? "" : "s");
   return(ss.str());
}
...
Date Person::GetBirthDay(){ return(bday);}
