...
class Date
{
	...
	public:
			...
			int GetYear(){ int y = static_cast<int>(ymd.year()); return(y);}
			int GetMonth() { unsigned m = static_cast<unsigned>(ymd.month()); return(int(m));}
			int GetDay(){ unsigned d = static_cast<unsigned>(ymd.day()); return(int(d));}
